...
// Вспомогательные функции для получения списка аргументов команды format
void argsToArray(std::string*& arr, int idx)
{
}

template <typename T, typename... Args>
void argsToArray(std::string*& arr, int idx, T head, Args... args)
{
    arr[idx++] = toString(head);
    argsToArray(arr, idx, args...);
}
...
std::string format(const std::string& format, const Args& ... args)
{
	...
	std::string* strs = new std::string[argsCount];
